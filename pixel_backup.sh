...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'EOF'
pixel_backup.sh — Resumable Pixel (Android) → Mac backup over USB/adb

USAGE
  ./pixel_backup.sh [--help]

  All configuration is done via environment variables, not command-line flags.
  Set them inline or export them before running.

  DEVICE_SERIAL=59110DLCQ002SF ./pixel_backup.sh
  DEST_ROOT_BASE=~/Backups PROGRESS_EVERY_FILES=100 ./pixel_backup.sh

━━━ DEVICE SELECTION ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  DEVICE_SERIAL          Serial number of the device to target.
                         If unset, DEVICE_SELECTION controls which device(s)
                         are used.              default: (unset — auto)

  DEVICE_SELECTION       How to pick when multiple devices are connected.
                           auto   — use DEVICE_SERIAL if set, else first online device
                           first  — always use the first online device
                           all    — back up every connected device in sequence
                                                default: auto

  SEPARATE_DEVICE_DIRS   1 = each device gets its own dated subfolder
                             <DEST_ROOT_BASE>/YYYY-MM-DD_DeviceName_Serial/
                         0 = all devices write to DEST_ROOT_BASE directly
                             (not recommended with multiple devices)
                                                default: 1
...
━━━ FOLDERS TO COPY ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  REMOTE_DIRS_CSV        Colon-separated list of folders to copy from the
                         phone, relative to the storage root.
                         Storage root is auto-detected (/sdcard or
                         /storage/emulated/0).
                         Example: REMOTE_DIRS_CSV="DCIM:Pictures:Movies"
                                                default: DCIM:Pictures:Movies:Download

━━━ RELIABILITY ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  MAX_PULL_ATTEMPTS      Retry count per file before marking it failed.
                                                default: 3

  MAX_ADB_RECOVERY_ATTEMPTS  How many times to restart the adb server if the
                         connection is unhealthy before giving up.
                                                default: 3

  ADB_WAIT_SECONDS       Seconds to wait for device to come online during
                         recovery.             default: 20

━━━ DISK SPACE SAFETY ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  PRECHECK_FREE_SPACE    0 = skip all pre-copy space checks
                         1 = fast: ensure FREE_SPACE_BUFFER_GB is available
                             before starting (no full file scan)
                         2 = full: scan all pending files, estimate total bytes,
                             then confirm destination has enough space
                             (slow on large libraries — can take minutes)
                                                default: 1

  FREE_SPACE_BUFFER_GB   GB that must be free before copying starts (mode 1),
                         or buffer added on top of the pending-bytes estimate
                         (mode 2).             default: 10

  PRECHECK_MAX_SECONDS   Timeout for the full pending-bytes scan (mode 2).
                         If exceeded, the check is skipped with a warning.
                                                default: 120

  CHECK_FREE_SPACE_DURING_COPY
                         1 = check remaining disk space periodically while
                         copying and warn / stop if it runs low.
                                                default: 1

  RUNTIME_FREE_SPACE_WARN_GB
                         Print a WARN when free space drops below this.
                                                default: 15

  RUNTIME_FREE_SPACE_STOP_GB
                         Soft-pause the transfer (exit 75) when free space
                         drops below this — progress is saved for resume.
                                                default: 2

━━━ PROGRESS & FEEDBACK ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  PROGRESS_EVERY_FILES   Print a PROGRESS summary line every N files.
                                                default: 200

  SHOW_RUNTIME_HINTS     1 = print contextual hints (USB mode, phone locked,
                         disk space) during the transfer.
                                                default: 1

  HEALTHCHECK_INTERVAL_SECONDS
                         How often (seconds) to check device health and emit
                         hints while copying.  default: 30

━━━ OUTPUT FORMAT ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Each log line is prefixed with a timestamp and a level tag:
    OK       file copied successfully
    COPY     file copy attempt started
    SKIP     file already present with matching size — not re-copied
    FAIL     file failed all retry attempts
    PROGRESS running totals (seen / copied / skipped / failed / GB)
    HINT     operator guidance (USB mode, phone lock, disk space)
    WARN     non-fatal warning
    ERROR    recoverable error (adb glitch, partial file)
    FATAL    unrecoverable error — script aborts

━━━ EXAMPLES ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  # Back up DCIM only to an external drive
  DEST_ROOT_BASE=/Volumes/SSD/Backups REMOTE_DIRS_CSV="DCIM" ./pixel_backup.sh

  # Target a specific phone (useful when two devices are connected)
  DEVICE_SERIAL=59110DLCQ002SF ./pixel_backup.sh

  # Back up all connected devices, print progress every 50 files
  DEVICE_SELECTION=all PROGRESS_EVERY_FILES=50 ./pixel_backup.sh

  # Dry-run style: disable disk precheck, get a summary of what would happen
  PRECHECK_FREE_SPACE=0 CHECK_FREE_SPACE_DURING_COPY=0 ./pixel_backup.sh
